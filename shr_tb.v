...
module shr_tb;
    ...
    DataPath DUT(
        ...
        //In
        .MARin(MARIn), .Zin(RZLoIn), 
        .PCin(PCIn), .MDRin(MDRIn), .IRin(IRIn), .Yin(RYIn),
    ...
    always @(Present_state) begin
        ...
        case (Present_state)
            ...
            //Fetch
            T0: begin 
                PCout <= 1; MARIn <= 1; IncPC <= 1; RZLoIn <= 1;
                #20 PCout <= 0; MARIn <= 0; IncPC <= 0; RZLoIn <= 0;
            end
            T1: begin 
                ZLoOut <= 1; PCIn <= 1; Read <= 1; MDRIn <= 1;
                
                Mdatain <= 32'h4b820000; 
                #20 ZLoOut <= 0; PCIn <= 0; Read <= 0; MDRIn <= 0;
            end
            T2: begin 
                MDRout <= 1; IRIn <= 1;
                #20 MDRout <= 0; IRIn <= 0;
